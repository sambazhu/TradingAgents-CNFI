        ...
        try:
            ...
            if is_china:
                ...
                try:
                    ...
                    # ★ Gildata 基本面数据富集（仅 A 股，替换 N/A + 附加新章节）
                    try:
                        from tradingagents.dataflows.gildata_fundamentals import enrich_fundamentals_report
                        fundamentals_data = enrich_fundamentals_report(
                            ticker, curr_date, fundamentals_data, data_depth=data_depth
                        )
                        logger.info(f"★ [Gildata] 基本面数据富集完成: {ticker}")
                    except ImportError:
                        logger.debug(f"[Gildata] gildata_fundamentals 模块不可用")
                    except Exception as e:
                        logger.debug(f"[Gildata] 基本面数据富集失败，不影响主报告: {e}")
...
**分析日期**: {curr_date}
"""
                    # ★ A 股：接入 Gildata + AKShare 情绪数据富集
                    if is_china:
                        try:
                            from tradingagents.dataflows.gildata_sentiment import enrich_sentiment_report
                            sentiment_summary = enrich_sentiment_report(ticker, curr_date, sentiment_summary)
                            logger.info(f"★ [情绪富集] A股情绪数据富集完成: {ticker}")
                        except ImportError:
                            logger.debug(f"[情绪富集] gildata_sentiment 模块不可用")
                        except Exception as e:
                            logger.debug(f"[情绪富集] A股情绪数据富集失败，不影响主流程: {e}")
