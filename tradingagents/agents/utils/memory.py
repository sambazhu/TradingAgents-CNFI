...
class FinancialSituationMemory:
    ...
    def _is_length_error(self, error_text: str) -> bool:
        error_text = (error_text or "").lower()
        return any(keyword in error_text for keyword in [
            'length', 'token', 'limit', 'exceed', 'too long', 'maximum', 'context'
        ])

    def _is_transient_error(self, error_text: str) -> bool:
        error_text = (error_text or "").lower()
        return any(keyword in error_text for keyword in [
            'ssl', 'eof', 'connection', 'timeout', 'tempor', 'reset by peer',
            ...
            'nodename nor servname provided', 'service not known', 'dns'
        ])

    def _get_dashscope_embedding_with_retry(self, text):
        max_attempts = max(1, int(os.getenv('DASHSCOPE_EMBEDDING_MAX_RETRIES', '3')))
        retry_delay = float(os.getenv('DASHSCOPE_EMBEDDING_RETRY_DELAY', '1.5'))
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if self._is_length_error(error_msg):
                    logger.warning(f"⚠️ DashScope长度限制: {error_msg}")
                    return None

                last_error = RuntimeError(error_msg)
                if attempt < max_attempts and self._is_transient_error(error_msg):
                    logger.warning(f"⚠️ DashScope embedding瞬时异常，第{attempt}/{max_attempts}次重试: {error_msg}")
                ...
                last_error = e
                error_str = str(e)
                if self._is_length_error(error_str):
                    logger.warning(f"⚠️ DashScope长度限制异常: {error_str}")
                    return None
                if attempt < max_attempts and self._is_transient_error(error_str):
                    logger.warning(f"⚠️ DashScope embedding网络异常，第{attempt}/{max_attempts}次重试: {error_str}")
                    time.sleep(retry_delay * attempt)
                    continue
                raise
    ...
    def get_embedding(self, text):
        ...
        if (self.llm_provider == "dashscope" or
            ...
            try:
                ...
                embedding = self._get_dashscope_embedding_with_retry(text)
