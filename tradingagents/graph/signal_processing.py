...
请确保：
1. action字段必须是"买入"、"增持"、"持有"、"减持"或"卖出"之一（绝对不允许使用英文buy/hold/sell）
...
如果某些信息在报告中没有明确提及，请使用合理的默认值。""",
        ...
        try:
            ...
            if json_match:
                ...
                if action not in ['买入', '增持', '持有', '减持', '卖出']:
                    # 尝试映射英文和其他变体
                    action_map = {
    ...
    def _extract_simple_decision(self, text: str) -> dict:
        ...
        if re.search(r'买入|BUY', text, re.IGNORECASE):
            action = '买入'
        elif re.search(r'增持|OVERWEIGHT|加仓|超配', text, re.IGNORECASE):
            action = '增持'
        elif re.search(r'减持|UNDERWEIGHT|减仓|低配', text, re.IGNORECASE):
            action = '减持'
