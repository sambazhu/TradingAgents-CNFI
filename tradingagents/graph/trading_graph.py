...
def create_llm_by_provider(provider: str, model: str, backend_url: str, temperature: float, max_tokens: int, timeout: int, api_key: str = None, **extra_kwargs):
    ...
    if provider.lower() == "google":
        ...
        # Forward provider-specific thinking params (深度思考机制)
        if provider.lower() == "openai":
            reasoning_effort = extra_kwargs.get("reasoning_effort")
            if reasoning_effort:
                llm_kwargs["reasoning_effort"] = reasoning_effort
                logger.info(f"🧠 [OpenAI] 启用深度思考: reasoning_effort={reasoning_effort}")
        ...
        # Forward Anthropic effort control (深度思考机制)
        effort = extra_kwargs.get("effort")
        if effort:
            llm_kwargs["effort"] = effort
            logger.info(f"🧠 [Anthropic] 启用深度思考: effort={effort}")
...
class TradingAgentsGraph:
    ...
    def _get_provider_kwargs(config: Dict[str, Any]) -> Dict[str, Any]:
        """Get provider-specific kwargs for LLM client creation (深度思考参数)."""
        kwargs = {}
        ...
        elif provider in ("openai", "siliconflow", "openrouter"):
    ...
    def __init__(
        ...
        if quick_provider and deep_provider and quick_provider != deep_provider:
            ...
            # 获取深度思考参数
            provider_kwargs = self._get_provider_kwargs(self.config)
            if provider_kwargs:
                logger.info(f"🧠 [深度思考] 参数: {provider_kwargs}")
            ...
            if anthropic_effort:
                anthropic_kwargs["effort"] = anthropic_effort
                logger.info(f"🧠 [Anthropic] 启用深度思考: effort={anthropic_effort}")
        ...
        self.graph = self.graph_setup.setup_graph(
            selected_analysts,
            include_risk=self.config.get("include_risk", True),
        )
