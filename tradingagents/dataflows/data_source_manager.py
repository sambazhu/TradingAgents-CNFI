...
class ChinaDataSource(Enum):
    ...
    GILDATA = DataSourceCode.GILDATA  # 恒生聚源 Gildata
...
class DataSourceManager:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                source_mapping = {
                    ...
                    DataSourceCode.GILDATA: ChinaDataSource.GILDATA,
                }
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                enabled_sources_in_db = {'mongodb', 'tushare', 'akshare', 'baostock', 'gildata'}
        except Exception as e:
        ...
        # 检查Gildata
        if 'gildata' in enabled_sources_in_db:
            try:
                from tradingagents.dataflows.gildata_client import is_gildata_enabled
                if is_gildata_enabled():
                    available.append(ChinaDataSource.GILDATA)
                    logger.info("✅ Gildata数据源可用且已启用")
                else:
                    logger.warning("⚠️ Gildata数据源不可用: Token未配置（环境变量和数据库均未找到）")
            except ImportError:
                logger.warning("⚠️ Gildata数据源不可用: 模块未安装")
            except Exception as e:
                logger.warning(f"⚠️ Gildata数据源不可用: {e}")
        else:
            logger.info("ℹ️ Gildata数据源已在数据库中禁用")
    ...
    def _format_stock_data_response(self, data: pd.DataFrame, symbol: str, stock_name: str,
        ...
        try:
            ...
            # ★ Gildata 预计算技术指标覆盖（仅 A 股，且 Gildata 已启用时）
            try:
                from tradingagents.utils.stock_utils import StockUtils
                market_info = StockUtils.get_market_info(symbol)
                if market_info.get('is_china'):
                    from tradingagents.dataflows.gildata_enrichment import apply_gildata_tech_indicators
                    gildata_applied = apply_gildata_tech_indicators(data, symbol, end_date)
                    if gildata_applied:
                        logger.info(f"★ [Gildata] 预计算技术指标已覆盖自算值: {symbol}")
                    else:
                        logger.debug(f"[Gildata] 技术指标覆盖未生效，使用自算值: {symbol}")
            except ImportError:
                logger.debug(f"[Gildata] gildata_enrichment 模块不可用")
            except Exception as e:
                logger.debug(f"[Gildata] 技术指标覆盖失败，使用自算值: {e}")
            ...
            # ★ Gildata 数据富集（估值、资金流向、风险指标，仅 A 股）
            try:
                from tradingagents.utils.stock_utils import StockUtils
                market_info = StockUtils.get_market_info(symbol)
                if market_info.get('is_china'):
                    from tradingagents.dataflows.gildata_enrichment import enrich_market_report
                    result = enrich_market_report(symbol, end_date, result)
                    logger.info(f"★ [Gildata] 数据富集完成: {symbol}")
            except ImportError:
                logger.debug(f"[Gildata] gildata_enrichment 模块不可用")
            except Exception as e:
                logger.debug(f"[Gildata] 数据富集失败，不影响主报告: {e}")
