...
from tradingagents.dataflows.gildata_enrichment import parse_markdown_table, find_column, safe_float
# 导入日志模块
...
"""
        ...
        is_degraded = bool(financial_estimates.get('is_degraded'))
        degraded_reason = financial_estimates.get('degraded_reason', '')

        if any("（估算值）" in str(v) for v in financial_estimates.values() if isinstance(v, str)):
            data_source_note = "\n⚠️ **数据说明**: 部分财务指标为估算值，建议结合最新财报数据进行分析"
        elif data_source == "Gildata":
            data_source_note = "\n⚠️ **数据说明**: 财务指标由 Gildata fallback 提供，部分字段可能缺失，建议结合最新财报复核"
        elif is_degraded:
            data_source_note = "\n⚠️ **数据说明**: 当前报告使用结构化降级财务数据，缺失字段已保留为 N/A"
...
"""
    ...
    def _build_degraded_financial_metrics(self, symbol: str, reason: str, data_source: str = "Degraded") -> dict:
        """构建结构化降级财务指标，避免上层因异常退化为纯错误文本。"""
        logger.warning(f"⚠️ [基本面降级] {symbol} 使用结构化降级财务指标: {reason}")
        return {
            "total_mv": "N/A",
            "pe": "N/A",
            "pe_ttm": "N/A",
    ...
    def _parse_gildata_financial_metrics(
        ...
        def _get_row(table_markdown: str):
            rows = parse_markdown_table(table_markdown)
            if len(rows) < 2:
                return None, None
            return rows[0], rows[1]

        def _get_num(headers, data, keywords):
        ...
        try:
            ...
            headers, data = _get_row(fa) if fa else (None, None)
            if headers and data:
                source_hits.append("FinancialAnalysis")

                roe = _get_num(headers, data, ['净资产收益率', 'ROE'])
                if roe is not None:
                    metrics["roe"] = f"{roe:.1f}%"

                roa = _get_num(headers, data, ['总资产收益率', '总资产报酬率', 'ROA'])
                if roa is not None:
                    metrics["roa"] = f"{roa:.1f}%"

                gross_margin = _get_num(headers, data, ['毛利率', '销售毛利率'])
                if gross_margin is not None:
                    metrics["gross_margin"] = f"{gross_margin:.1f}%"

                net_margin = _get_num(headers, data, ['净利率', '销售净利率', '净利润率'])
                if net_margin is not None:
                    metrics["net_margin"] = f"{net_margin:.1f}%"

                debt_ratio = _get_num(headers, data, ['资产负债率'])
                if debt_ratio is not None:
                    metrics["debt_ratio"] = f"{debt_ratio:.1f}%"

                current_ratio = _get_num(headers, data, ['流动比率'])
                if current_ratio is not None:
                    metrics["current_ratio"] = f"{current_ratio:.2f}"

                quick_ratio = _get_num(headers, data, ['速动比率'])
                if quick_ratio is not None:
                    metrics["quick_ratio"] = f"{quick_ratio:.2f}"

                cash_ratio = _get_num(headers, data, ['现金比率'])
                if cash_ratio is not None:
                    metrics["cash_ratio"] = f"{cash_ratio:.2f}"

                eps = _get_num(headers, data, ['EPS', '每股收益', '基本每股收益'])
                if eps is not None and eps > 0 and price_value > 0 and "pe" not in metrics:
                    metrics["pe"] = f"{price_value / eps:.1f}倍"

        except Exception as e:
            logger.warning(f"⚠️ [Gildata-基本面] 财务分析解析失败 {symbol}: {e}")

        try:
            val = client.get_valuation(symbol, trade_date)
            headers, data = _get_row(val) if val else (None, None)
            if headers and data:
                source_hits.append("StockValueAnalysis")

                pe = _get_num(headers, data, ['PE(静)', '市盈率'])
                if pe is not None and pe > 0:
                    metrics["pe"] = f"{pe:.1f}倍"

                pe_ttm = _get_num(headers, data, ['PE(TTM)'])
                if pe_ttm is not None and pe_ttm > 0:
                    metrics["pe_ttm"] = f"{pe_ttm:.1f}倍"

                pb = _get_num(headers, data, ['市净率PB', 'PB'])
                if pb is not None and pb > 0:
                    metrics["pb"] = f"{pb:.2f}倍"

                ps = _get_num(headers, data, ['PS(TTM)', '市销率'])
                if ps is not None and ps > 0:
                    metrics["ps"] = f"{ps:.2f}倍"

                dividend_yield = _get_num(headers, data, ['股息率'])
                if dividend_yield is not None:
                    metrics["dividend_yield"] = f"{dividend_yield:.2f}%"

                total_mv = _get_num(headers, data, ['总市值（万元）', '总市值'])
                if total_mv is not None and total_mv > 0:
                    metrics["total_mv"] = f"{total_mv / 1e4:.2f}亿元"
        except Exception as e:
            logger.warning(f"⚠️ [Gildata-基本面] 估值解析失败 {symbol}: {e}")

        if not metrics:
            logger.info(f"ℹ️ [Gildata-基本面] 未解析到可用指标: {symbol}")
            return None

        metrics.setdefault("total_mv", "N/A")
        metrics.setdefault("pe", "N/A")
        metrics.setdefault("pe_ttm", "N/A")
        metrics.setdefault("pb", "N/A")
        metrics.setdefault("ps", "N/A")
        metrics.setdefault("dividend_yield", "N/A")
        metrics.setdefault("roe", "N/A")
        metrics.setdefault("roa", "N/A")
        metrics.setdefault("gross_margin", "N/A")
        metrics.setdefault("net_margin", "N/A")
        metrics.setdefault("debt_ratio", "N/A")
        metrics.setdefault("current_ratio", "N/A")
        metrics.setdefault("quick_ratio", "N/A")
        metrics.setdefault("cash_ratio", "N/A")
        metrics.setdefault("fundamental_score", 6.5)
        metrics.setdefault("valuation_score", 6.0)
        metrics.setdefault("growth_score", 6.0)
        metrics.setdefault("risk_level", "中等")
        metrics["data_source"] = "Gildata"
        metrics["is_degraded"] = True
        metrics["degraded_reason"] = f"Gildata fallback after primary financial sources unavailable; hits={','.join(source_hits) if source_hits else 'none'}"
    ...
    def _estimate_financial_metrics(self, symbol: str, current_price: str, analysis_date: str = "") -> dict:
        ...
        error_msg = f"无法获取股票 {symbol} 的财务数据。已尝试所有数据源（MongoDB、Gildata、AKShare、Tushare）均失败。"
        logger.error(f"❌ {error_msg}")
