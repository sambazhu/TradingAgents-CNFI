...
**初始状态**：
- MongoDB `deepseek` 厂家：`api_key = "sk-your-deepseek-key-example"`
...
**初始状态**：
- MongoDB `dashscope` 厂家：`api_key = ""` 或 `null`
- .env 文件：`DASHSCOPE_API_KEY=sk-your-dashscope-key-example`
...
1. 访问"设置 → 大模型厂家管理"
2. 点击"编辑" `dashscope` 厂家
3. 填写新的 API Key：`sk-your-new-api-key-example`
4. 点击"保存"
5. 访问"设置 → 配置验证"
6. 点击"验证配置"按钮
