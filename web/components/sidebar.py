...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Gildata（恒生聚源）
        gildata_key = os.getenv("GILDATA_API_TOKEN")
        status, level = validate_api_key(gildata_key, "gildata")
        if level == "success":
            st.success(f"✅ Gildata: {status}")
        elif level == "warning":
            st.warning(f"⚠️ Gildata: {status}")
        else:
            st.info("ℹ️ Gildata: 未配置")
        ...
        **数据源**: Tushare + AKShare + Gildata + FinnHub API
        """)
