...
const getResearchDebateRounds = (depth: number) => {
  if (depth <= 3) {
    return 1
  }
  if (depth === 4) {
...
const buildAnalysisSteps = () => {
  const steps: Array<{ key: string; title: string; description: string; status: string }> = [
    {
      key: 'prepare',
      title: '📋 准备阶段',
  ...
  const analystStepMap: Record<string, { key: string; title: string; description: string }> = {
    市场分析师: {
      key: 'market',
      title: '📊 市场分析师',
  ...
  analysisForm.selectedAnalysts.forEach((analystName) => {
    const step = analystStepMap[analystName]
    if (step) {
  ...
  const debateRounds = getResearchDebateRounds(analysisForm.researchDepth)
  for (let index = 0; index < debateRounds; index += 1) {
    steps.push({
      key: `debate_${index + 1}`,
...
const SOCIAL_ANALYST_NAME = '社媒分析师'
const hasSocialAnalystSelected = () => analysisForm.selectedAnalysts.includes(SOCIAL_ANALYST_NAME)
...
const updateProgressInfo = (status: any) => {
  ...
  if (analysisSteps.value.length === 0) {
    analysisSteps.value = buildAnalysisSteps()
    console.log('📋 根据当前配置生成步骤列表:', analysisSteps.value.length, '个步骤')
  }
...
const updateAnalysisSteps = (status: any) => {
  ...
  const normalizeStepTitle = (value: string) =>
    value.replace(/正在分析|构建论据|识别风险|形成共识|制定策略/g, '').trim()

  const currentStepName = normalizeStepTitle(
    status.current_step_name ||
    progressInfo.value.currentStep ||
    status.message ||
    ''
  )
  ...
  if (currentStepName) {
    currentStepIndex = analysisSteps.value.findIndex((step) => {
    ...
    console.log('📋 使用步骤名称匹配索引:', currentStepIndex, '名称:', currentStepName)
  }
