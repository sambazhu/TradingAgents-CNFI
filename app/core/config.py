...
class Settings(BaseSettings):
    ...
    # ===== Gildata（恒生聚源）数据源配置 =====
    GILDATA_API_TOKEN: str = Field(default="", description="Gildata API Token")
    GILDATA_API_BASE_URL: str = Field(
        default="https://api.gildata.com/mcp-servers/aidata-assistant-srv-api",
        description="Gildata API 基础URL"
    )
    GILDATA_TIMEOUT: int = Field(default=30, description="Gildata API 超时时间（秒）")
    GILDATA_CALL_INTERVAL: float = Field(default=3.0, description="Gildata API 调用间隔（秒）")

    @field_validator("DEBUG", mode="before")
    @classmethod
    def _parse_debug_value(cls, value):
        """兼容 DEBUG=release / production 等历史写法，避免启动阶段直接报错。"""
